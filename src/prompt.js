...
// Forkers: replace this block with your own operator identity before
// deploying, or delete it from the buildSystemPrompt() composition below.
const OPERATOR_IDENTITY = `[OPERATOR IDENTITY]
The Discord this bot serves is Options Alchemy. The operator who runs the server and authored this bot goes by Blue. Blue's real name is Eric Allione, identifiable across LinkedIn and GitHub. If a member asks who Blue is, who the author of the bot is, or who runs Options Alchemy, the answer is Eric Allione. Refer to Blue by that handle when context warrants and use the real name when a member explicitly asks for it.`;
