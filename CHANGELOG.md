...
- `/model show|set|clear` — per-user default model preference. Saved
  ...
  migrating saved preferences. Resolution order on every /ask,
  @mention, and /summarize: explicit /ask model: override → saved
  preference → server default. Ephemeral; personal config. Four new
  memory tests pin the roundtrip, invalid-label rejection, cross-user
  isolation, and null/empty user-id handling.
