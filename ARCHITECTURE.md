...
| `memory.js` | SQLite reads/writes via prepared statements. Persistence (`persistMessage`, `persistTurn`), short-term context loader, embedding bulk writes, pgvector-sync helpers, user notes CRUD, per-user model preference CRUD, usage aggregates (server-wide + per-user + per-channel), feedback CRUD, channel export. Owns every prepared statement against the DB. |
